...
func (p *Postgres) GetLinkedAccounts(email string) ([]types.Account, error) {
	db, err := p.GetDB()
	if err != nil {
		return nil, err
	}
	...
	res, err := db.Query(
		...
		"SELECT a.account_id, a.account_name, a.account_email, a.account_type, a.account_password, a.account_locked, "+
			...
			"a.account_id = l.sub_account_id JOIN account b ON l.main_account_id=b.account_id WHERE b.account_email=$1 "+
...
func (p *Postgres) LinkAccounts(main types.Account, sub types.Account) error {
	...
	_, err = tx.Exec(
		ctx,
		"INSERT INTO linked_accounts(main_account_id, sub_account_id) VALUES ($1,$2) "+
...
func (p *Postgres) UnlinkAccounts(main types.Account, sub types.Account) error {
	db, err := p.GetDB()
	if err != nil {
		return err
	}
	...
	_, err = db.Exec(
		ctx,
		"DELETE FROM linked_accounts WHERE main_account_id=$1 AND sub_account_id=$2;",
		main.Identifier,
		sub.Identifier,
	)
	if err != nil {

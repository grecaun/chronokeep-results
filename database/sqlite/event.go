...
func (s *SQLite) GetAccountEvents(email string) ([]types.Event, error) {
...
func (s *SQLite) DeleteEvent(event types.Event) error {
	db, err := s.GetDB()
	if err != nil {
		return err
	}
...
func (s *SQLite) RealDeleteEvent(event types.Event) error {
	db, err := s.GetDB()
	if err != nil {
		return err
	}
...
func (s *SQLite) UpdateEvent(event types.Event) error {
	db, err := s.GetDB()
	if err != nil {
		return err
	}

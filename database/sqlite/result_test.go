...
func TestAddResults(t *testing.T) {
	...
	results[0].Division = "UpdatedDivisions!"
	results[0].DivisionRanking = 100
	...
	assert.Equal(t, results[0], res[0])
	...
	results[1].Division = "AnotherUpdate!"
	...
	if !res[0].SamePerson(&(results[1])) {
		t.Errorf("Expected to find matching personal information. Expected %v, found %v.", results[1], res[0])
	}

...
func (m *MySQL) AddSubscribedPhone(eventYearID int64, subscription types.SmsSubscription) error {
	db, err := m.GetDB()
	if err != nil {
		return err
	}
...
func (m *MySQL) GetSubscribedPhones(eventYearID int64) ([]types.SmsSubscription, error) {
	db, err := m.GetDB()
	if err != nil {
		return nil, err
	}

...
// GetDistanceResults Gets the distance results (last only) for a distance.
func (m *MySQL) GetAllDistanceResults(eventYearID int64, distance string, limit, page int) ([]types.Result, error) {
	return m.getResultsInternal(eventYearID, nil, All, distance, limit, page)
}
...
func (m *MySQL) DeleteResults(eventYearID int64, results []types.Result) (int64, error) {
	db, err := m.GetDB()
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, fmt.Errorf("unable to get prepared statement for result deletion: %v", err)
	}
	defer stmt.Close()
	for _, result := range results {
		...
		if err != nil {
			tx.Rollback()
			return 0, fmt.Errorf("error executing prepared delete statement: %v", err)
		}
	}
	err = tx.Commit()
	if err != nil {
...
func (m *MySQL) DeleteDistanceResults(eventYearID int64, distance string) (int64, error) {
	db, err := m.GetDB()
	if err != nil {
		return 0, err
	}
	...
	res, err := tx.ExecContext(
		ctx,
		"DELETE r FROM result AS r WHERE EXISTS (SELECT * FROM person AS p WHERE p.event_year_id=? AND p.distance=? AND p.person_id=r.person_id);",
		eventYearID,
		distance,
	)
	if err != nil {
	...
	if err != nil {
		tx.Rollback()
		return 0, fmt.Errorf("error fetching rows affected from event year & distance results deletion: %v", err)
	}
	_, err = tx.ExecContext(
		ctx,
		"DELETE FROM person WHERE event_year_id=? AND distance=?;",
		eventYearID,
		distance,
	)
	if err != nil {
